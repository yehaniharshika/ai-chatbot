    ...
    <div className={`flex gap-4 message-enter ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      <div
        className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center shadow-md ${
          isUser
            ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-[12px]'
            : 'bg-gradient-to-br from-gray-100 to-gray-200 text-[12px]'
        }`}
      >
        {isUser ? (
      ...
      <div className="flex flex-col max-w-[75%] sm:max-w-xl">
        ...
        >
          <p className="text-sm leading-relaxed whitespace-pre-wrap break-words text-[12px] font-medium">
            {message.text}
          </p>
        </div>
        <span
